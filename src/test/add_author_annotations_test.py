import unittest
import pandas as pd
from cas.add_author_annotations import add_author_annotations, validate_columns, dataframe_to_dict
...
class TestAuthorAnnotations(unittest.TestCase):
    def setUp(self):
        ...
        self.cas = {
            "annotations": [
                {"labelset": "set1", "cell_label": "101", "cell_set_accession": "c_1", "author_annotation_fields": {}},
                {"labelset": "set2", "cell_label": "202", "cell_set_accession": "c_2", "author_annotation_fields": {}}
            ]
        }
    ...
    def test_single_column_filter(self):
        ...
        result = dataframe_to_dict(self.df, 'cell_set_accession', 'set1', 'c_1', ['extra_data'])
        self.assertEqual(result, {'extra_data': 'value1'})

    def test_multi_column_filter(self):
        # Testing with multiple column filters
        result = dataframe_to_dict(self.df, ['labelset', 'cell_label'], 'set1', '101', ['extra_data'])
        self.assertEqual(result, {'extra_data': 'value1'})
    ...
    def test_without_specified_columns(self):
        ...
        self.assertEqual(result, {'labelset': 'set1', 'cell_label': '101', 'extra_data': 'value1'})
    ...
    def test_invalid_join_column(self):
        ...
        with self.assertRaises(ValueError):
            dataframe_to_dict(self.df, 123, 'set1', 'c_1', ['extra_data'])  # join_column is neither str nor list

    def test_no_matching_rows(self):
        # Testing filter that results in no matching rows
        result = dataframe_to_dict(self.df, 'cell_set_accession', 'set1', '999', ['extra_data'])
    ...
    def test_add_author_annotations(self):
        ...
        updated_cas = add_author_annotations(self.cas, self.df, 'cell_set_accession', ['extra_data'])
        self.assertIn('value1', updated_cas['annotations'][0]['author_annotation_fields']['extra_data'])
        self.assertIn('value2', updated_cas['annotations'][1]['author_annotation_fields']['extra_data'])
